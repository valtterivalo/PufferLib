...
def test_osrs_web_build_links_vsync_library(tmp_path: Path) -> None:
    shutil.copy(REPOSITORY_ROOT / "build.sh", tmp_path / "build.sh")
    ...
    result = subprocess.run(
        ["bash", "build.sh", "osrs_inferno", "--web"],
        cwd=tmp_path,
        env=environment,
        capture_output=True,
        text=True,
    ...
    assert result.returncode == 0, result.stderr
    arguments = compiler_arguments.read_text().splitlines()
    assert arguments[arguments.index("--js-library") + 1] == "vendor/puf_web_vsync.js"