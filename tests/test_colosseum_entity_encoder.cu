...
void colo_entity_test_init(int B, int obs_size, int hidden, int mode) {
    ...
    // The encoder mode knob (0=flat, 1=NPC pool, 2=+inventory) was deleted in the
    // 5c port; both pooled branches are now unconditional for osrs_colosseum. The
    // parameter is retained so the ctypes signature stays stable.
    (void)mode;
    create_custom_encoder("osrs_colosseum", &g_enc);
