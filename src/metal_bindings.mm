...
static void rollouts(py::object pufferl_obj) {
    ...
    if (pufferl.phase2_ctx) phase2_apply_cursor_gate(pufferl.phase2_ctx);
}
...
static std::unique_ptr<PuffeRL> create_pufferl(py::dict args) {
    ...
    float parent_kl_coef = get_config(train_kwargs, "parent_kl_coef");
    bool parent_kl_log = get_config(train_kwargs, "parent_kl_log") > 0;
