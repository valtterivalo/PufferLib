...
struct Muon {
    ...
    FloatTensor mb_puf, row_stats_puf;
    PrecisionTensor gram, gram_buf, x_buf, orig_buf;
    FloatTensor aurora_row_scale;
...
void muon_post_create(Muon* m) {
    ...
    if (m->row_stats_puf.data) m->row_stats_ptr = m->row_stats_puf.data;
