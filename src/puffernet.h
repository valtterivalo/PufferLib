...
// Inference mirror of ocean.cu's ColosseumEntityEncoder: output = global(flat obs)
...
// mode 2 inventory-cell pool (mirrors ocean.cu COLO_ENT_INV_* / models.py)
...
void forward_puffernet(PufferNet* net, float* observations, float* actions) {
    float* encoded;
    if (net->entity_encoder) {
        colosseum_entity_encoder(net->entity_encoder, observations);
        encoded = net->entity_encoder->output;
    } else {
        linear(net->encoder, observations);
        encoded = net->encoder->output;
    }
    mingru(net->mingru, encoded);
