...
static inline int archive_export_top_k_demos(
    ...
    for (int rank = 0; rank < n_candidates && written < max_demos; rank++) {
        ...
        Phase2DemoHeader h = {
            ...
            .hidden_state_size = (uint32_t)a->hidden_state_size,
            .reserved = 0,
        };
        ...
        if (ok && a->hidden_state_size > 0) {
            for (int s = 0; s < n_cells && ok; s++) {
                const void* hidden = archive_get_hidden_state(a, chain_cells[s]);
                ok = hidden && fwrite(hidden, 1, a->hidden_state_size, fp) ==
                    a->hidden_state_size;
            }
        }
