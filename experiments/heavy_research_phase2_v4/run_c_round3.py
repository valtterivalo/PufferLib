"""Stage C round 3: iterated phase 2 from p2k4szzs.

Round 2 found nsf=0.65 was the sweet spot (both seeds hit 0.78), better than
nsf=0.75 which was best in round 1. Suggests pushing phase 2 weight harder
when demos are strong.

Round 3 design:
  proposal: p2k4szzs final ckpt (50M, score 0.779)
  demos: refreshed from p2k4szzs (seed_43 archive, q up to 0.868)
  arms: 8 cells, all phase 2 no BC
    nsf in {0.50, 0.55, 0.60, 0.65} x 2 seeds = 8

Run from repo root:
  python experiments/heavy_research_phase2_v4/run_c_round3.py
"""
...
LOG_BASE = REPO / "experiments/heavy_research_phase2_v4/c_round3_logs"
TIMESTEPS = 50_000_000

SCHEDULE = [
    {"label": "p2-nsf50", "seed": 1, "nsf": 0.50},
    {"label": "p2-nsf50", "seed": 2, "nsf": 0.50},
    {"label": "p2-nsf55", "seed": 1, "nsf": 0.55},
    {"label": "p2-nsf55", "seed": 2, "nsf": 0.55},
    {"label": "p2-nsf60", "seed": 1, "nsf": 0.60},
    {"label": "p2-nsf60", "seed": 2, "nsf": 0.60},
...
def cli_for_cell(cell):
    tag = f"stage-r2-c-rd3-{cell['label']}-s{cell['seed']}"
    log_subdir = f"r2_c_rd3/{cell['label']}_s{cell['seed']}"
...
def main():
    ...
    print(f"C round 3: {len(SCHEDULE)} cells, ~{len(SCHEDULE) * 4} min total")
    if args.dry_run:
        print("\n--- DRY RUN ---\n")
    failures = []
    ...
    print(f"\n=== C round 3 done. {len(SCHEDULE) - len(failures)}/{len(SCHEDULE)} succeeded ===")
    for f in failures:
        print(f"  FAILED: {f}")
