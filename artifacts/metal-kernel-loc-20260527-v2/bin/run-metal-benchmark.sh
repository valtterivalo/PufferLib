...
cp "$REPO_ROOT/config/default.ini" "$CONFIG_DIR/default.ini"
if [ -f "$REPO_ROOT/config/$ENV_NAME.ini" ]; then
    cp "$REPO_ROOT/config/$ENV_NAME.ini" "$CONFIG_DIR/$ENV_NAME.ini"
fi
if [ -f "$REPO_ROOT/config/ocean/$ENV_NAME.ini" ]; then
    mkdir -p "$CONFIG_DIR/ocean"
    cp "$REPO_ROOT/config/ocean/$ENV_NAME.ini" "$CONFIG_DIR/ocean/$ENV_NAME.ini"
fi
...
python3 - "$OUT_DIR/eval.log" "$OUT_DIR/eval-summary.json" <<'PY'
import json
import sys
