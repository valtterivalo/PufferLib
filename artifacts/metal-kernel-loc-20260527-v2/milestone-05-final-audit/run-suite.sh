...
"$SCRIPT_DIR/run-breakout.sh"
"$SCRIPT_DIR/run-g2048.sh"
"$SCRIPT_DIR/../bin/run-gpu-inference-smoke.sh" milestone-05-final-audit
cd "$REPO_ROOT"
...
"$SCRIPT_DIR/../bin/check-median-gate.py" "$ARTIFACT_ROOT/milestone-04-second-kernel-loc" "$SCRIPT_DIR" loc
"$SCRIPT_DIR/../bin/run-interactive-smoke.sh" milestone-05-final-audit final
