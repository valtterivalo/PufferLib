...
"$SCRIPT_DIR/run-breakout.sh"
"$SCRIPT_DIR/run-g2048.sh"
"$SCRIPT_DIR/../bin/run-gpu-inference-smoke.sh" milestone-03-cleanup
cd "$REPO_ROOT"
...
"$SCRIPT_DIR/../bin/check-median-gate.py" "$ARTIFACT_ROOT/milestone-02-kernel-consolidation" "$SCRIPT_DIR" loc
