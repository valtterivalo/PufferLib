...
- Subagent review: Anscombe found no blocking findings, verified the Metal source files were restored to the 04w state, checked benchmark arithmetic and rejection gate, confirmed 04x runners are scoped and non-recursive, checked exact overlay allowlist entries, and confirmed generated benchmark artifacts are ignored rather than staged.
- Decision: rejected. The candidate exceeded the 1 percent SPS gate in both benchmark envs, so `src/metal/shader_src.h` and `src/metal/kernels.mm` were restored to the 04w accepted state. Only rejection artifacts, runner scripts, overlay allowlist, and this status entry remain for audit.

## 2026-05-27 Milestone 04y Float Count Param Sharing Candidate

- Root-cause hypothesis: `FillParams`, `ScaleParams`, `AxpyParams`, `NesterovParams`, and `NormalizeParams` all encode the same `float` plus `int` constant-buffer layout. Sharing one `FloatCountParams` layout should preserve buffer bytes, binding indices, dispatch counts, optimizer math, RNG, PPO data flow, and exported kernel names while deleting duplicate shader parameter structs.
- Candidate source change: replace those five MSL structs with one `FloatCountParams`, update live shader field reads from per-operation names to `value`, and keep all host payload structs unchanged because Metal constant buffers are layout-bound, not field-name-bound. Add dedicated milestone 04y runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is `2,237`, down from accepted `2,257`. `src/metal/kernels.mm` is unchanged at `1,457`. Full `src/metal/platform.mm` is unchanged at `1,088`. Kernel scope is `3,811`, down from accepted `3,831` and setup `4,615`. Backend scope is `9,442` after overlay allowlist growth, down from `9,458` after the rejected 04x audit commit.
- Risk classification: medium. This changes live shader source for fill, scale, axpy, Nesterov, and normalize kernels, but preserves constant-buffer ABI and arithmetic. Acceptance requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04y-float-count-param-sharing` runners, require repeated medians within 1 percent of the current accepted 04w baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.
- Harness note: first 04y suite attempt wrote `20260527T062823+0300-g2048` and failed before training with `ValueError: num_threads must be positive` because the new 04y g2048 runner omitted the small benchmark overrides used by the prior g2048 milestone runners. The runner was fixed before source acceptance testing continued.

## 2026-05-27 Milestone 04y Float Count Param Sharing Results

- Candidate code change: temporarily shared one `FloatCountParams` MSL struct across fill, scale, axpy, Nesterov, and normalize. The candidate would have reduced `src/metal/shader_src.h` from `2,257` to `2,237` and kernel scope from `3,831` to `3,811`, but it is not kept.
- Static validation passed before benchmark rejection: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04y runners, and `PYTHONPATH=$PWD python -m pytest tests/metal/test_overlay_surface.py`.
- Excluded failed-suite breakout artifact: `20260527T062812+0300-breakout` at `3,435,507` SPS, train score `2.5192012786865234`, eval score `0.0`. This source-equivalent breakout run is recorded but excluded from acceptance medians because its paired g2048 run failed from the runner override bug above.
- First corrected milestone 04y suite: `breakout` artifact `20260527T062937+0300-breakout` at `3,325,989` SPS, train score `2.5987749099731445`, eval score `0.0`. `g2048` artifact `20260527T062949+0300-g2048` at `589,406` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second corrected milestone 04y suite: `breakout` artifact `20260527T063012+0300-breakout` at `3,394,284` SPS, train score `2.0698659420013428`, eval score `0.0`. `g2048` artifact `20260527T063024+0300-g2048` at `599,230` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Medians: corrected `breakout` two-run median `3,360,137` SPS, `-2.53%` versus milestone 04w accepted median `3,447,380`. Corrected `g2048` two-run median `594,318` SPS, `-0.36%` versus milestone 04w accepted median `596,463`.
...
## 2026-05-27 Milestone 04aa Dead Include Cleanup Candidate

- Root-cause hypothesis: the monolithic MSL source no longer uses Metal atomics after earlier dead-helper cleanup, and `src/metal/platform.mm` no longer uses `CACurrentMediaTime`. Removing `#include <metal_atomic>` and `#import <QuartzCore/CABase.h>` should preserve compiled live kernels, exported names, dispatch order, RNG, PPO math, and eval behavior while deleting dead include lines.
- Candidate source change: remove `#include <metal_atomic>` from `src/metal/shader_src.h` and remove the unused QuartzCore import from `src/metal/platform.mm`. Add dedicated milestone 04aa runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is `2,256`, down from accepted `2,257`. `src/metal/kernels.mm` is unchanged at `1,445`. Full `src/metal/platform.mm` is `1,087`, down from accepted `1,088`. Kernel scope is `3,818`, down from accepted `3,819` and setup `4,615`. Backend scope is `9,456` after overlay allowlist growth, up from accepted `9,454` because the two source-line deletion is smaller than the four-line runner allowlist.
- Risk classification: low. This removes apparently unused imports, but the raw shader source string changes, so acceptance still requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04aa-dead-include-cleanup` runners, require repeated medians within 1 percent of the current accepted 04z baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.

## 2026-05-27 Milestone 04aa Dead Include Cleanup Results

- Code change: remove unused `#include <metal_atomic>` from the embedded MSL source and unused `#import <QuartzCore/CABase.h>` from `src/metal/platform.mm`.
- LOC: `src/metal/shader_src.h` is `2,256`, down from accepted `2,257`. `src/metal/kernels.mm` is unchanged at `1,445`. Full `src/metal/platform.mm` is `1,087`, down from accepted `1,088`. Kernel scope is `3,818`, down from accepted `3,819` and setup `4,615`. Backend scope is `9,456`, up from accepted `9,454` because the two source-line deletion is smaller than the four-line runner allowlist.
- Static validation passed: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04aa runners, and `PYTHONPATH=$PWD python -m pytest -p no:cacheprovider tests/metal/test_overlay_surface.py`.
- First milestone 04aa suite: `breakout` artifact `20260527T064403+0300-breakout` at `3,434,241` SPS, train score `2.616342306137085`, eval score `0.0`. `g2048` artifact `20260527T064414+0300-g2048` at `609,678` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second milestone 04aa suite: `breakout` artifact `20260527T064436+0300-breakout` at `3,412,722` SPS, train score `2.1889469623565674`, eval score `0.0`. `g2048` artifact `20260527T064447+0300-g2048` at `598,306` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Medians: `breakout` two-run median `3,423,482` SPS, `+0.12%` versus milestone 04z accepted median `3,419,458`. `g2048` two-run median `603,992` SPS, `+0.75%` versus milestone 04z accepted median `599,512`.
- Native Metal parity and overlay surface passed in both milestone 04aa suites.
- Interactive smoke artifact: `milestone-04aa-dead-include-cleanup/20260527T064743+0300-interactive-breakout-dead-include-cleanup`. It built, loaded `resources/breakout/breakout_weights.bin`, reached raylib 5.5 GLFW initialization, then failed with `Failed to determine Monitor to center Window` and exit code `139`, matching the known desktop windowing boundary.
- Subagent review: Huygens found no blocking findings, verified the immutable plan has no diff, checked the source scope, confirmed no MSL atomics or QuartzCore/CABase/CACurrentMediaTime references remain, checked runner scope and g2048 overrides, verified benchmark arithmetic, checked parity, overlay, and interactive artifact claims, and confirmed generated artifacts are ignored rather than staged.
- Decision: accepted after subagent review. The cleanup clears the throughput gate on both benchmark envs and preserves train, eval, parity, overlay, and interactive boundary behavior.

## 2026-05-27 Milestone 04ab Tensor Ops Wrapper Macro Candidate

- Root-cause hypothesis: the Metal 4 tensor-ops shader block still carries six exported kernel wrappers that differ only by exported name, scalar type, and typed helper body. A single MSL preprocessor macro can generate the same six exported kernel names while preserving buffer slots, constant bindings, template helper calls, dispatch dimensions, tensor-ops math, RNG, PPO data flow, and eval behavior.
- Candidate source change: replace the six explicit `tensor_ops_gemm_*_{f32,f16}` exported wrappers in the tensor-ops MSL source string with one `TENSOR_OPS_GEMM_KERNEL` macro plus six invocations. Add dedicated milestone 04ab runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is unchanged at `2,256`. `src/metal/kernels.mm` is unchanged at `1,445`. The `src/metal/platform.mm` tensor-ops shader block is `83` lines, down from accepted `117`. Full `src/metal/platform.mm` is `1,057`, down from accepted `1,087`. Kernel scope is `3,784`, down from accepted `3,818` and setup `4,615`. Backend scope is `9,430` after overlay allowlist growth, down from accepted `9,456`.
- Risk classification: medium. This changes live tensor-ops shader source and relies on MSL preprocessing, but generated exported names and call targets should be identical. Acceptance requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04ab-tensor-ops-wrapper-macro` runners, require repeated medians within 1 percent of the current accepted 04aa baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.

## 2026-05-27 Milestone 04ab Tensor Ops Wrapper Macro Results

- Candidate code change: temporarily replaced the six explicit tensor-ops exported wrappers in `src/metal/platform.mm` with one `TENSOR_OPS_GEMM_KERNEL` macro and six invocations. The candidate would have reduced the tensor-ops shader block from `117` to `83` lines, full `src/metal/platform.mm` from `1,087` to `1,057`, and kernel scope from `3,818` to `3,784`, but it is not kept.
- Static validation passed before benchmark rejection: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04ab runners, and `PYTHONPATH=$PWD python -m pytest -p no:cacheprovider tests/metal/test_overlay_surface.py`.
- First milestone 04ab suite: `breakout` artifact `20260527T065519+0300-breakout` at `3,417,255` SPS, train score `2.5078768730163574`, eval score `0.0`. `g2048` artifact `20260527T065530+0300-g2048` at `599,937` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second milestone 04ab suite: `breakout` artifact `20260527T065547+0300-breakout` at `3,457,641` SPS, train score `2.451415538787842`, eval score `0.0`. `g2048` artifact `20260527T065558+0300-g2048` at `593,554` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Extra g2048 runner: `20260527T065655+0300-g2048` at `597,747` SPS, train score `97.85507202148438`, eval score `49.43283462524414`. This extra run was added because the two-run g2048 median narrowly missed the 1 percent gate.
- Medians: `breakout` two-run median `3,437,448` SPS, `+0.41%` versus milestone 04aa accepted median `3,423,482`. `g2048` three-run median `597,747` SPS, `-1.03%` versus milestone 04aa accepted median `603,992`.
...
## 2026-05-27 Milestone 04ad Host Param Struct Reuse Candidate

- Root-cause hypothesis: `src/metal/kernels.mm` repeats anonymous C++ constant-buffer structs that have identical byte layouts across many live wrappers. Replacing those local anonymous structs with named host-only structs should preserve MSL source, exported shader names, host symbol names, constant-buffer bytes, binding indices, dispatch dimensions, command order, RNG, PPO math, and eval behavior while deleting repeated host source.
- Candidate source change: add five small host parameter structs in `src/metal/kernels.mm`, then reuse them for fill, clamp, scale, axpy, Nesterov, clip, normalize, transpose, decoder-gradient assembly, sum-rows, MinGRU gate, scan dispatch, Muon update, and anchor blending. Add dedicated milestone 04ad runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is unchanged at `2,256`. `src/metal/kernels.mm` is `1,413`, down from accepted `1,445`. Full `src/metal/platform.mm` is unchanged at `1,087`. Kernel scope is `3,786`, down from accepted `3,818` and setup `4,615`. Backend scope is `9,436` after overlay allowlist growth, down from `9,464` after the rejected 04ac audit commit.
- Risk classification: low to medium. This is host-only constant-buffer layout reuse, but it touches live wrappers feeding many shaders. Acceptance requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04ad-host-param-struct-reuse` runners, require repeated medians within 1 percent of the current accepted 04aa baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.

## 2026-05-27 Milestone 04ad Host Param Struct Reuse Results

- Code change: add five host-only parameter structs in `src/metal/kernels.mm` and replace repeated anonymous constant-buffer payload structs in live wrappers. Shader source, kernel names, bindings, dispatch dimensions, and command order are unchanged.
- LOC: `src/metal/shader_src.h` is unchanged at `2,256`. `src/metal/kernels.mm` is `1,413`, down from accepted `1,445`. Full `src/metal/platform.mm` is unchanged at `1,087`. Kernel scope is `3,786`, down from accepted `3,818` and setup `4,615`. Backend scope is `9,436`, down from `9,464` after the rejected 04ac audit commit.
- Static validation passed: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04ad runners, and `PYTHONPATH=$PWD python -m pytest -p no:cacheprovider tests/metal/test_overlay_surface.py`.
- First milestone 04ad suite: `breakout` artifact `20260527T071535+0300-breakout` at `3,502,740` SPS, train score `2.723320245742798`, eval score `0.0`. `g2048` artifact `20260527T071546+0300-g2048` at `591,332` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second milestone 04ad suite: `breakout` artifact `20260527T071600+0300-breakout` at `3,390,785` SPS, train score `2.3482210636138916`, eval score `0.0`. `g2048` artifact `20260527T071611+0300-g2048` at `610,004` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Medians: `breakout` two-run median `3,446,763` SPS, `+0.68%` versus milestone 04aa accepted median `3,423,482`. `g2048` two-run median `600,668` SPS, `-0.55%` versus milestone 04aa accepted median `603,992`.
- Native Metal parity passed and wrote `milestone-04ad-host-param-struct-reuse/native-metal.json`. Overlay surface passed in both milestone 04ad suites, then was rerun with captured stdout and exit code `0` at `milestone-04ad-host-param-struct-reuse/20260527T072100+0300-overlay-surface`.
- Interactive smoke artifact: `milestone-04ad-host-param-struct-reuse/20260527T071854+0300-interactive-breakout-host-param-struct-reuse`. It built, loaded `resources/breakout/breakout_weights.bin`, reached raylib 5.5 GLFW initialization, then failed with `Failed to determine Monitor to center Window` and exit code `139`, matching the known desktop windowing boundary.
- Subagent review: Tesla found one blocking audit gap: overlay surface had passed but lacked a standalone captured stdout artifact in the 04ad milestone folder. The blocker was fixed by rerunning overlay surface and capturing `20260527T072100+0300-overlay-surface/output.log` plus `exit-code.txt`. Tesla then re-reviewed and found no blockers, verified the immutable plan has no diff, checked host struct layout equivalence, source scope, benchmark arithmetic, native parity, ignored generated artifacts, and the interactive boundary artifact.
- Decision: accepted after subagent review. The cleanup clears the throughput gate on both benchmark envs and preserves train, eval, parity, overlay, and interactive boundary behavior.

## 2026-05-27 Milestone 04ae MinGRU Wrapper Macro Candidate

- Root-cause hypothesis: `src/metal/shader_src.h` already shares MinGRU scan math through templated bodies, but still repeats eight long exported wrapper signatures for fp32 and fp16 scan variants. Local MSL macros can generate the same exported kernel names, argument address spaces, buffer indices, template-body calls, dispatch dimensions, RNG flow, PPO math, and train/eval behavior while deleting wrapper boilerplate.
- Candidate source change: remove explicit `mingru_scan_*` fp32 and fp16 wrapper bodies and replace them with four local wrapper macros plus eight invocations. The four templated scan bodies are unchanged. Add dedicated milestone 04ae runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is `2,162`, down from accepted `2,256`. `src/metal/kernels.mm` is unchanged at `1,413`. Full `src/metal/platform.mm` is unchanged at `1,087`. Kernel scope is `3,692`, down from accepted `3,786` and setup `4,615`. Backend scope is `9,346` after overlay allowlist growth, down from accepted `9,436`.
- Risk classification: medium. This changes live MSL source and relies on preprocessing for MinGRU scan exported kernels, including fp16 training paths. Acceptance requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04ae-mingru-wrapper-macros` runners, require repeated medians within 1 percent of the current accepted 04ad baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.

## 2026-05-27 Milestone 04ae MinGRU Wrapper Macro Results

- Code change: replace eight explicit MinGRU scan exported wrapper bodies with four local MSL wrapper macros and eight invocations. The templated scan math bodies, exported kernel names, buffer indices, host dispatch code, dispatch dimensions, and command order are unchanged.
- LOC: `src/metal/shader_src.h` is `2,162`, down from accepted `2,256`. `src/metal/kernels.mm` is unchanged at `1,413`. Full `src/metal/platform.mm` is unchanged at `1,087`. Kernel scope is `3,692`, down from accepted `3,786` and setup `4,615`. Backend scope is `9,346`, down from accepted `9,436`.
- Static validation passed: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04ae runners, and `PYTHONPATH=$PWD python -m pytest -p no:cacheprovider tests/metal/test_overlay_surface.py`.
- First milestone 04ae suite: `breakout` artifact `20260527T073455+0300-breakout` at `3,429,222` SPS, train score `2.5934579372406006`, eval score `0.0`. `g2048` artifact `20260527T073506+0300-g2048` at `609,114` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second milestone 04ae suite: `breakout` artifact `20260527T073520+0300-breakout` at `3,423,714` SPS, train score `2.192251443862915`, eval score `0.0`. `g2048` artifact `20260527T073531+0300-g2048` at `614,549` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Medians: `breakout` two-run median `3,426,468` SPS, `-0.59%` versus milestone 04ad accepted median `3,446,763`. `g2048` two-run median `611,832` SPS, `+1.86%` versus milestone 04ad accepted median `600,668`.
- Native Metal parity passed and wrote `milestone-04ae-mingru-wrapper-macros/native-metal.json`. Overlay surface passed in both milestone 04ae suites, then was rerun with captured stdout and exit code `0` at `milestone-04ae-mingru-wrapper-macros/20260527T073600+0300-overlay-surface`.
- Interactive smoke artifact: `milestone-04ae-mingru-wrapper-macros/20260527T073612+0300-interactive-breakout-mingru-wrapper-macros`. It built, loaded `resources/breakout/breakout_weights.bin`, reached raylib 5.5 GLFW initialization, then failed with `Failed to determine Monitor to center Window` and exit code `139`, matching the known desktop windowing boundary.
- Subagent review: Banach found no blockers, verified the immutable plan has no diff, checked source scope, macro-generated fp32 and fp16 exported wrapper equivalence, buffer indices, address spaces, unused reset buffers, body calls, unchanged host dispatch code, LOC arithmetic, benchmark arithmetic, native parity, captured overlay, interactive boundary artifact, and ignored generated artifacts.
- Decision: accepted after subagent review. The wrapper macro cleanup clears the throughput gate on both benchmark envs and preserves train, eval, parity, overlay, and interactive boundary behavior.

## 2026-05-27 Milestone 04af Small Wrapper Macro Candidate

- Root-cause hypothesis: `src/metal/shader_src.h` still repeats paired exported wrappers around unchanged shared bodies for transpose, cast, and Steel GEMM paths. Local MSL wrapper macros can generate the same exported kernel names, argument address spaces, buffer indices, template-body calls, threadgroup memory declaration, dispatch dimensions, and train/eval behavior while deleting wrapper boilerplate.
- Candidate source change: replace explicit `transpose_01` and `transpose_01_u64` wrappers with `TRANSPOSE_01_KERNEL`, replace `cast_f32_to_f16` and `cast_f16_to_f32` wrappers with `CAST_KERNEL`, and replace `steel_gemm` and `steel_gemm_f16` wrappers with `STEEL_GEMM_KERNEL`. Shared bodies and host dispatch code are unchanged. Add dedicated milestone 04af runners and exact overlay allowlist paths.
- LOC before validation: `src/metal/shader_src.h` is `2,129`, down from accepted `2,162`. `src/metal/kernels.mm` is unchanged at `1,413`. Full `src/metal/platform.mm` is unchanged at `1,087`. Kernel scope is `3,659`, down from accepted `3,692` and setup `4,615`. Backend scope is `9,317` after overlay allowlist growth, down from accepted `9,346`.
- Risk classification: medium. This changes live MSL source and uses macros for exported wrappers on transpose, fp16 cast, and GEMM fallback paths. Acceptance requires build success, repeated `breakout` and `g2048`, native parity, overlay surface, interactive smoke, comparable train and eval scores, and subagent review.
- Acceptance gate: use the dedicated `milestone-04af-small-wrapper-macros` runners, require repeated medians within 1 percent of the current accepted 04ae baseline or better, require train and eval scores to remain comparable, and reject on compile, parity, determinism, interactive, or score drift.

## 2026-05-27 Milestone 04af Small Wrapper Macro Results

- Candidate code change: temporarily replaced explicit transpose, cast, and Steel GEMM exported wrappers with local MSL wrapper macros. The candidate would have reduced `src/metal/shader_src.h` from `2,162` to `2,129` and kernel scope from `3,692` to `3,659`, but it is not kept.
- Static validation passed before benchmark rejection: `tools/metal/build.sh breakout`, `tools/metal/build.sh g2048`, `git diff --check`, `bash -n` for all milestone 04af runners, and `PYTHONPATH=$PWD python -m pytest -p no:cacheprovider tests/metal/test_overlay_surface.py`.
- First milestone 04af suite: `breakout` artifact `20260527T074308+0300-breakout` at `3,484,123` SPS, train score `2.5524256229400635`, eval score `0.0`. `g2048` artifact `20260527T074320+0300-g2048` at `602,688` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Second milestone 04af suite: `breakout` artifact `20260527T074335+0300-breakout` at `3,424,928` SPS, train score `2.460716962814331`, eval score `0.0`. `g2048` artifact `20260527T074345+0300-g2048` at `604,718` SPS, train score `97.85507202148438`, eval score `49.43283462524414`.
- Medians: `breakout` two-run median `3,454,526` SPS, `+0.82%` versus milestone 04ae accepted median `3,426,468`. `g2048` two-run median `603,703` SPS, `-1.33%` versus milestone 04ae accepted median `611,832`. The 1 percent g2048 floor is `605,714`, so an extra run cannot rescue the median because both completed g2048 runs are below the floor.
- Overlay surface passed in both milestone 04af suites.
- Subagent review: Noether found no blockers, verified the immutable plan has no diff, confirmed `src/metal/shader_src.h` was restored with no current `src/metal` diff, checked benchmark arithmetic and rejection, confirmed a third g2048 run cannot rescue the median, reran `git diff --check`, `bash -n`, and overlay surface after restore, and confirmed generated 04af artifacts are ignored.
- Decision: rejected. The candidate exceeds the 1 percent g2048 SPS gate, so `src/metal/shader_src.h` was restored to the 04ae accepted state. Only rejection artifacts, runner scripts, overlay allowlist, and this status entry remain for audit.
