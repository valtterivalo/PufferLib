...
static void zul_write_action_mask_bytes(Env* env, unsigned char* mask_out) {
    if (!mask_out) return;
    float mask_f[ZUL_ACTION_MASK_SIZE];
    ENCOUNTER_ZULRAH.write_mask(ZUL_ENV_STATE(env), ZUL_ENV_CONTEXT(env), mask_f);
...
void puf_reset(Env* env) {
    ...
    zul_write_action_mask_bytes(env, agent->action_mask);
}
