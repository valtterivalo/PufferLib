 ...
 * @brief Item database for the shared OSRS layer.
 *
 * Originally generated by ocean/osrs/tools/generate_items.py from
 * osrs-dps-calc equipment.json plus items_manifest.json. Neither the
 * generator nor the manifest exists on this branch, so this file is now
 * maintained by hand: add items directly and set each item's stats and
 * effect_mask in place.
 *
 * The effect_mask column has no other source of truth, so it is guarded by
 * ocean/osrs/tests/test_osrs_item_effect_masks.c, which pins the exact
 * item to effect mapping. Update that test in the same change whenever you
 * add, remove, or change an item effect.
 */
...
typedef enum {
    ...
    ITEM_VENATOR_BOW = 142,  /* Venator bow */
    ITEM_ABYSSAL_TENTACLE = 143,  /* Abyssal tentacle */
    NUM_ITEMS = 144,
    ITEM_NONE = 255
} ItemIndex;
