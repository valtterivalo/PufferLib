...
def _parse_modern_item_entry(item_id: int, data: bytes) -> ItemDef:
    ...
    while True:
        ...
        if opcode == 0:
            ...
            op_id = buf.read(1)[0]
            while True:
                subop_id = buf.read(1)[0] - 1
                if subop_id == -1:
                    break
                _read_string(buf)
        elif opcode == 44:
            d.inv_model = struct.unpack(">I", buf.read(4))[0]
        elif opcode == 45:
            d.male_wield = struct.unpack(">I", buf.read(4))[0]
            d.male_offset = struct.unpack(">B", buf.read(1))[0]
        elif opcode == 46:
            d.male_wield2 = struct.unpack(">I", buf.read(4))[0]
        elif opcode == 47:
            d.male_wield3 = struct.unpack(">I", buf.read(4))[0]
        elif opcode == 48:
            buf.read(4)  # femaleWield
            buf.read(1)  # femaleOffset
        elif opcode == 49:
            buf.read(4)  # femaleWield2
        elif opcode == 50:
            buf.read(4)  # femaleWield3
        elif opcode == 51:
            buf.read(4)  # maleHeadModel
        elif opcode == 52:
            buf.read(4)  # maleHeadModel2
        elif opcode == 53:
            buf.read(4)  # femaleHeadModel
        elif opcode == 54:
            buf.read(4)  # femaleHeadModel2
        elif opcode == 62:
            buf.read(1)  # unknown u8
        elif opcode == 64:
...
class ModelData:
    ...
    face_alpha_labels: list[int] = field(default_factory=list)  # per-face anim alpha label group (255=none)
...
def _decode_face_skins(
    data: bytes, offset: int, count: int, has_face_skins: int,
) -> list[int]:
    """Read per-face animation alpha label groups.

    The cache stores a faceSkins byte per face when var15 == 1. The renderer
    keys type-5 alpha transforms on these labels (Scythe swing trail, Shockwave
    Colossus shimmer). Faces with no group decode to 255.
    """
    if has_face_skins == 1:
        return [_read_ubyte(data, offset + i) for i in range(count)]
    return [255] * count
...
def _decode_type2(model_id: int, data: bytes) -> ModelData | None:
    ...
    face_alpha_labels = _decode_face_skins(data, var27, var10, var15)
    ...
    return ModelData(
        ...
        face_alphas=alphas,
        face_alpha_labels=face_alpha_labels,
...
def _decode_old_format(model_id: int, data: bytes) -> ModelData | None:
    ...
    face_alpha_labels = _decode_face_skins(data, var25, var10, var15)
...
def _decode_type1(model_id: int, data: bytes) -> ModelData | None:
    ...
    face_alpha_labels = _decode_face_skins(data, var30, var10, var15)
...
def _decode_type3(model_id: int, data: bytes) -> ModelData | None:
    ...
    face_alpha_labels = _decode_face_skins(data, var32, var10, var15)
...
def _merge_models(models: list[ModelData]) -> ModelData:
    ...
    for md in models:
        ...
        for face_idx in range(md.face_count):
            ...
            merged.face_alpha_labels.append(
                md.face_alpha_labels[face_idx]
                if face_idx < len(md.face_alpha_labels) else 255
            )
...
def write_models_binary(
    ...
    """Write models to .models binary format (MDL4, atlas-textured).

    MDL4 keeps the animation payload, adds expanded texcoords after colors,
    writes per-face alpha data, and requires an atlas. A sibling ATLS file is
    always written.
    ...
    Per-model binary layout:
      ...
      uint8  face_alphas[face_count]
      uint8  face_alpha_labels[face_count]

    Raises:
        ValueError: if no atlas is supplied (MDL4 requires one).
    """
    if atlas is None:
        raise ValueError("write_models_binary: MDL4 requires an atlas")
        ...
        for model in models:
            ...
            if has_textures:
                for i in range(model.face_count):
                    alpha = (
                        model.face_alphas[i]
                        if i < len(model.face_alphas) else 0
                    )
                    f.write(struct.pack("B", max(0, min(255, alpha))))
                for i in range(model.face_count):
                    label = (
                        model.face_alpha_labels[i]
                        if i < len(model.face_alpha_labels) else 255
                    )
                    f.write(struct.pack("B", max(0, min(255, label))))
...
def main() -> None:
    ...
    for idx, item_id in enumerate(SIM_ITEM_IDS):
        ...
        has_sleeves = 1 if item.male_wield2 >= 0 or item.male_wield3 >= 0 else 0
