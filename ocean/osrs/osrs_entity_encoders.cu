// OSRS entity encoders (colosseum + inferno). Included by src/ocean.cu.
// Both envs share the weight and activation layout, so they live together.
...
static PrecisionTensor inf_entity_encoder_forward(void* w, void* activations, PrecisionTensor input, cudaStream_t stream) {
    ColosseumEntityEncoderWeights* ew = (ColosseumEntityEncoderWeights*)w;
    ColosseumEntityEncoderActivations* a = (ColosseumEntityEncoderActivations*)activations;
    int B = input.shape[0];
    int H = ew->hidden;
    ...
    PrecisionTensor npc2d = {.data = a->npc_flat.data, .shape = {NB, INF_ENT_FEATS}};
    puf_mm(&npc2d, &ew->entity_l1_w, &a->entity_z1, stream);
    colo_ent_launch_fused_fwd(
        a->out.data, a->pool_argmax.data, a->entity_h1.data,
        a->entity_z1.data, a->npc_flat.data,
    ...
    PrecisionTensor inv2d = {.data = a->inv_flat.data, .shape = {IB, INF_ENT_INV_FEATS}};
    puf_mm(&inv2d, &ew->inv_l1_w, &a->inv_z1, stream);
    colo_ent_launch_fused_fwd(
        a->out.data, a->inv_pool_argmax.data, a->inv_h1.data,
        a->inv_z1.data, a->inv_flat.data,
...
static void inf_entity_encoder_backward(void* w, void* activations, PrecisionTensor grad, cudaStream_t stream) {
    ColosseumEntityEncoderWeights* ew = (ColosseumEntityEncoderWeights*)w;
    ColosseumEntityEncoderActivations* a = (ColosseumEntityEncoderActivations*)activations;
    int B = grad.shape[0];
    int H = ew->hidden;
