...
def _params_from_puffer_sweep(sweep_config, only_include=None, path=''):
    param_spaces = {}

    if 'sweep_only' in sweep_config:
        only_include = {p.strip().replace('.', '/') for p in sweep_config['sweep_only'].split(',')}

    for name, param in sweep_config.items():
        if name in ('method', 'metric', 'metric_distribution', 'goal', 'downsample', 'use_gpu', 'prune_pareto',
                    'sweep_only', 'max_suggestion_cost', 'early_stop_quantile', 'min_sps', 'gpus', 'max_runs'):
            continue
        ...
        if any(isinstance(param[k], dict) for k in param):
            nested_spaces = _params_from_puffer_sweep(param, only_include, full_name)
            if nested_spaces:
...
class Protein:
    ...
    def suggest(self, fill, fixed_total_timesteps=None):
        ...
        if len(suggestions) == 0:
            return self.suggest(fill, fixed_total_timesteps=fixed_total_timesteps) # Fallback to random if all suggestions are filtered
